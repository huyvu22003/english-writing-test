...
| `AUDIO_REVIEW_MIN` | Listening: số phút rà soát sau khi hết audio rồi tự nộp |
| `BAND_TABLE` | Bảng quy đổi raw→band IELTS Academic (reading/listening) |
| `FALLBACK_QUIZ` | Đề trắc nghiệm DEMO khi chạy offline (kèm đáp án để chấm thử) |

Chi tiết deploy: `docs/deployment.md` (frontend) và `docs/google-apps-script.md` (backend).

---

## 8. Chế độ TRẮC NGHIỆM Reading/Listening (Phase 1)

> Chi tiết phương án: `docs/READING-LISTENING-PLAN.md`. Nhập đề: `docs/reading-listening-sample.md`.

- **Nhận diện**: topic có tên bắt đầu `RL-` → `state.mode = "quiz"` → app vào màn `quizScreen` (thay vì `writeScreen`). Hàm `isQuizTopic()`.
- **Tái dùng** toàn bộ hạ tầng phần Viết: khóa fullscreen, ghi log vi phạm, đồng hồ **giờ thực**, lưu nháp/khôi phục (`saveSession`/`tryRestore` đã có nhánh quiz).
- **Tải đề** (`fetchQuiz`): online gọi `action=quiz` (backend trả đề **KHÔNG kèm đáp án**); offline lấy `FALLBACK_QUIZ` (tách đáp án ra `state.quizKey` để chấm thử tại máy).
- **Render**: `renderQuizLeft` (passage Reading / audio Listening) + `renderQuestion` (4 dạng Phase 1: single/multi/tfng/fill) + `renderNavigator` (lưới câu, tô màu đã làm/đánh dấu/đang xem) + `updateProgress` (x/40).
- **Audio Listening** (`setupAudio`): nghe 1 lần, chặn tua (chặn `seeking` vượt mốc đã nghe); hết audio → `onAudioEnded` cho `AUDIO_REVIEW_MIN` phút rồi tự nộp.
- **Chấm** (`gradeQuiz`): online gửi `action=grade` (server đối chiếu đáp án trong Sheet, lưu `BaiNop-RL`, email GV+HS, trả điểm/band/từng câu); offline `gradeLocally` + `bandFromRaw`. Màn `resultScreen` hiện điểm thô + band + bảng từng câu.
- **Bảo mật**: đáp án (cột F của Sheet) **không bao giờ** rời server trước khi nộp. Chấm điểm ở backend.
- **Hạn chế Phase 1**: chưa có dạng `match`/`complete` (nhập tạm bằng ô text), chưa xáo đáp án, chưa bảng tổng hợp lớp; khôi phục phiên Listening sẽ phát lại audio từ đầu.
